...
export default function LoginPage() {
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    // Save newly created user account into valid system users array dynamically
  ...
  const completeSocialAuth = (role: UserRole) => {
    const socialUser: SystemUser = {
      role,
      name: `${socialProvider} User (${roleDetails[role].labelEn})`,
      email: `${socialProvider?.toLowerCase()}.${role}@safarload.pk`,
  ...
  return (
    <div className={styles.container} dir={lang === 'ur' ? 'rtl' : 'ltr'}>
      ...
      {/* Social Login Role Picker Modal */}
      {socialModalOpen && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h3>🌐 {socialProvider} Authentication</h3>
              <button className={styles.closeBtn} onClick={() => setSocialModalOpen(false)}>✕</button>
            </div>
            <p className={styles.modalSubtitle}>
              You are signing in with <strong>{socialProvider}</strong>. Select your account role to continue:
            </p>
            ...
            <div className={styles.socialRoleList}>
              {(Object.keys(roleDetails) as UserRole[]).map((roleKey) => (
                <button
                  ...
                  onClick={() => completeSocialAuth(roleKey)}
                >
                  <span>{roleDetails[roleKey].icon}</span>
                  <div>
